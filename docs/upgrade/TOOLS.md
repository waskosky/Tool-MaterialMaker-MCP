...
# Reusable artwork, AI proposals and motion

`profile: "vector-document-v2"` adds to the document workflow. `describe` returns
installed templates (including `courier`), limits, the plant rest-pose adapter's
typed controls and AI configuration status. `list/get` can read original v1
artwork; v1 commands never silently mutate/read v2 projects. `upgrade` requires
`project_id`, `expected_revision` and `idempotency_key`. It preserves rest SVG and
records an undoable explicit transition. Undo/redo or named restore may cross the
version boundary through v2; legacy history rejects unsupported target versions
before writing.

V2 `patch` supports the RAI component/style/recipe/rig/clip edits. Definitions are
embedded, components cannot recurse, and all expanded art retains the original
bounds. Shared styles override appearance; detach a style before editing individual
appearance. Components generated by `plant-rest-v1` retain parameters and regenerate
from them; detach the recipe before direct part edits. This adapter uses detail zero
and the rest pose. Original plant animation remains in the plant recipe workflow.

`library_save {kind: component|style, definition_id}` requires the exact project
revision. It freezes source dependencies and producer under a content hash;
`library_list/get {library_id}` reads at most 128 immutable saved entries.
`library_import {library_id, idempotency_key, x?, y?, target?}` needs the exact
project revision. It remaps collisions and embeds copies; later source/library
changes never propagate into another artwork. A style may bind to `target`.

`ai_submit {intent, selected_ids}` captures the exact project revision and hash.
An empty selection means the whole document. One provider may run per workspace;
concurrent HTTP/MCP processes share a kernel lock. `ai_get {proposal_id}` reports
running/ready/failed/cancelled/accepted and supplies the validated candidate when
ready. `ai_cancel` cancels or discards it. `ai_accept` needs `proposal_id`,
`project_id`, `expected_revision` and `candidate_sha256`. It refuses stale sources,
checks the exact candidate, and returns the same receipt on retries. Only accept
writes. Proposal jobs are process-local, at most eight, expire after 15 minutes,
and disappear on restart. Accepted artwork/history remains durable.

The optional provider uses the host's authenticated Codex CLI with an explicit
binary/model/effort. No shell, web, app, plugin or agent tools are enabled. Runs
ignore user configuration/rules, are ephemeral/read-only, and time out at 180s.
A private worker ownership pipe kills its process group on service death; normal
cancellation/shutdown also cleans the group. Capture is bounded to 2MiB stdout and
128KiB stderr. Prompts/model output are not logged or written into the project
store. The provider never gets Workshop tokens or arbitrary API environment keys.
Malformed or out-of-scope edits fail with no fallback or writes. The external MCP
handoff continues to work when the provider is unconfigured.

`rig_suggest {preset: auto|flap|walk|sway|bob}` returns confidence/reasons, editable
hinges and a candidate clip. Named limbs are rigid parts: there is no automatic
segmentation or skinning. `preview_edits {operations}` validates candidate changes
without saving; accept through the usual revision-fenced patch. `sample {clip,time}`
and `frames {clip,count}` use deterministic smoothstep interpolation. Browser
playback is local; neither the service nor database is called per frame.

V2 `build {clip?}` freezes the explicit export selection. Rest builds retain three
source files plus manifest; motion builds add `motion.sheet.svg` and `motion.json`.
The selected clip uses 16 frames in four columns of 256px cells, with source hash,
exact sample times, loop flag and duration. The source retains editable rigs/keys.
The entire build is bounded to 4MiB; every read checks closed inventory, hashes and
regenerated art/motion. `build_get` includes the motion sheet/timing when present.
A browser can rasterize that same sheet to PNG; native game intake still requires
its own reviewed pin and rendering tests. This is asset authoring, not publishing.

### Editable arrangements and material masks

The v2 profile includes the `emblem` template and the RAI-owned
`modifier_create`, `modifier_update` and `modifier_source` patch edits. The
`vector-repeat-v1` component recipe retains source shapes and independent colors;
mirror, linear/radial repetition and seeded translation variation regenerate
bounded geometry. Generated bytes must match the source and settings exactly.
Existing project revisions, undo and explicit recipe detach apply unchanged.

`mask` requires the current project ID/revision and returns an opaque grayscale
rest silhouette. `build` optionally accepts `mask: true`; its frozen bundle adds
`mask.svg` and `mask.json`, bound to that exact source. `build_get` returns
`material_mask_svg` and `material_mask`. Mask and motion exports can coexist;
the eight-file maximum and four-MiB total budget are enforced. Existing export
requests omit the new field and preserve their original frozen inventory/bytes.

Foundry converts the mask to an explicitly sized PNG and imports it into its
existing material store. SVG source integrity and browser raster provenance are
separate facts; target native/Web material appearance remains a consumer gate.
The compiler projection now contains 17 reviewed files within 512 KiB.
