...
- [x] Add persisted native-path settings beneath the user's configuration directory
...
- [x] Detect common local Godot executables and adjacent Material Maker checkouts.
...
- [x] Apply saved paths to the existing service only when its queue is idle; rebuild
...
- [x] Add private server discovery and authenticated identity checks. Launch opens a
...
- [x] Add source launchers that prepare runtime dependencies and launch the browser;
  make `scripts/configure.py` support repair rather than refusing an existing file.
- [x] Run focused setup/HTTP/session tests and review both contract and quality.
...
- [x] Derive friendly recipe titles/descriptions from existing guides/metadata; expose
  authenticated thumbnails from completed builds without background mass rendering.
- [x] Add snapshot listing to GraphStore and the HTTP adapter.
- [x] Build a responsive gallery with search, categories, favorites and recipe cards;
  add setup status/editor, first-use guidance and automatic small previews.
- [x] Replace raw family JSON with typed range/lock controls and seed/count fields.
...
- [x] List and restore snapshots; show friendly job states and retry. Separate search
  debounce from preview debounce and keep project revision conflict handling intact.
- [x] Run focused browser regressions and exercise a real browser with screenshots;
